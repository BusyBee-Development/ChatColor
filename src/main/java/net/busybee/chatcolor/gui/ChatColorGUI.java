...
public class ChatColorGUI {
    ...
    public static void openAllPatterns(Player player, int page) {
        ConfigurationManager configurationManager = ChatColorPlugin.getInstance().getConfigurationManager();
        FileConfiguration file = configurationManager.getGui();
        FileConfiguration patternsConfig = configurationManager.getPatterns();
        List<BasePattern> allPatterns = ChatColorPlugin.getInstance().getPatternManager().getAllPatterns();
        ...
        String title = file.getString("gui.patterns.title", "&8Select Pattern (Page %page%)")
                .replace("%category%", "All Patterns")
        ...
        if (contentSlots.isEmpty()) {
            for (int s = 0; s < 45; s++) {
                if (s % 9 != 0 && s % 9 != 8 && s / 9 != 0) contentSlots.add(s);
            }
        }
        ...
        if (page > 1) {
            builder.setGuiItem(file.getInt("gui.patterns.prev-page-slot", 45),
                    ...
                    GuiClickActionManager.getClickActionFromString("OPEN-ALL-PAGE:" + (page - 1)));
        }
        if (page < totalPages) {
